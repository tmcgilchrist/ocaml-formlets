...
(*
   ...
   for lexing.  We need the power of a pushdown automaton, since we
   may have arbitrarily nested lexing contexts, as when we have
   expressions embedded in XML literals embedded in expressions etc.

   To handle this, we maintain a stack of lexing functions which
   encode the state.  When we enter a new context (e.g. an embedded
   XML literal) we push a lexer on the stack; when we reach the end of
   that context we pop from the stack and carry on where we were
   before.

   (The stack of lexers has other advantages: it makes it easy to
   return multiple tokens from a single match: see the definition and
   uses of the function `multiple'.)

   The general plan is as follows:
   1. when we see the beginning of a start tag in TOKEN,
      push STARTTAG onto the stack and return LXML id

   2. when we see an lbrace in token, push TOKEN onto the stack
      and return LBRACE

   3. when we see an rbrace in TOKEN, pop the stack and return RBRACE

   4. various sorts of pushing and popping in starttag

   5. to "run" the lexer, push token onto the stack and call the
      first thing on the stack

   Xml lexing functions:
     xml_start_tag (we've seen '<id' in token)
     xml_contents  (we've seen '>' in starttag)

   There are also various functions (comment, string, etc.) for lexing
   regular OCaml terms.
*)
...
module TokenEval = Struct.Token.Eval
...
= struct
  ...
  *)
  ...
  { loc        : Loc.t    ;
    ...
    lexbuf     : lexbuf   ;
    buffer     : Buffer.t ;
    lexers     : (string * (context -> Lexing.lexbuf -> Token.t)) Stack.t }
  ...
  let push_lexer
      (name : string)
      (lexer : context -> Lexing.lexbuf -> Token.t)
      (ctxt : context) : unit =
    Stack.push (name, lexer) ctxt.lexers
  ...
  rule token c = parse
    ...
    | "<#/>"                                                        { multiple c [KEYWORD ":xml"; KEYWORD "<#/>"] }
    | "<#>"                                                         { push_lexer "xml_contents" xml_contents c;
                                                                      multiple c [KEYWORD ":xml"; KEYWORD "<#>"]  }
    | "<" (lowercase identchar* as x)                               { push_lexer "xml_start_tag" xml_start_tag c;
                                                                      multiple c [KEYWORD ":xml"; LABEL x] }
    | "=>"                                                           { KEYWORD "FORMLET_ARROW" }
  ...
  and xml_start_tag c = parse
      (* we've seen '<id' in token; parse the rest of the opening tag  *)

      (* what can we see here?
          an identifier (attribute name)
          the "=" sign
          a string (attribute value)
          the /> closing thing
          the  > closing thing
          blanks
          comments (let's leave these for now)
      *)
    | ident * as x                  { STRING (x, x) }
    | "="                           { SYMBOL "="  }
    | '"'                           { with_curr_loc string c;
                                      let s = buff_contents c in STRING (TokenEval.string s, s) }
    | "/>"                          { (* fall back *)
                                      let () = pop_lexer c in
                                        SYMBOL "/>" }
    | ">"                           { (* switch to `xml_contents' *)
                                      let () = pop_lexer c in
                                      let () = push_lexer "xml_contents" xml_contents c in
                                      SYMBOL ">"  }
    | blank + as x                  { BLANKS x    }

  and xml_contents c = parse
      (* we've seen '>' in xml_start_tag; parse the contents of the element *)

       (* what can we see here?
           various "special characters" (&amp; etc.)
           escaped braces
           an opening brace
           a closing brace (error)
           cdata
           a closing tag
    ...
    | "</" (lowercase identchar* as x) '>' { let () = pop_lexer c in
                                               multiple c [SYMBOL "</";
                                                           LIDENT x;
                                                           SYMBOL ">"] }
    | '<'  (lowercase identchar* as x)     { (* switch to `xml_start_tag' to handle the nested xml, then back here *)
                                             let () = push_lexer "xml_start_tag" xml_start_tag c in
                                               LABEL x  }
  ...
  let from_context c =
    let () = push_lexer "token" token c in
    let next _ =
      let lexer_name, lexer = next_lexer c in
      let () = if debugging then prerr_endline ("lexer: " ^ lexer_name) in
      let tok = with_curr_loc lexer c in
